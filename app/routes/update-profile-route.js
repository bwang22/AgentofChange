...
router.post("/AOC/updateProfile/:username", async function(req, res) {
    var coords = null;
    if (req.body)
    ...
    var response = {}, code, newProfile = (new Profile(req.body)).toObject(), username = req.params.username;
    try {
        ...
        var count = (await profileService.updateProfile(username, newProfile)).modifiedCount;
        if (count === 0)
        ...
        logger.error(err, { note: "Error occurred while updating profile", data: username });
